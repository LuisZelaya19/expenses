    ...
    <div class="card-body">
        <form action="{{route('expenses.update', $expense->id)}}" method="post">
            ...
            <label for="name" class="block mt-2">
                ...
                <input id="name" name="name" class="{{ $errors->has('name') ? 'bg-red-100 ': ' ' }} block w-full mt-1 form-input" placeholder="Ingrese el nombre del gasto" value="{{old('name', $expense->name)}}">
            </label>
            @if ($errors->has('name'))
